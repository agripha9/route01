...
## §49 — 정적 약관/개인정보처리방침 페이지 + 마케팅 동의 제거 (2026-05-04)

### 목표
§48 마감 메모 1순위. Google OAuth 동의 화면에 supabase.co URL이 큰 글씨로 노출되는 신뢰 문제를 해결하기 위해, GCP 브랜딩 등록에 필요한 정적 약관/개인정보처리방침 페이지를 만든다. 동시에 약관 컨텐츠 자체가 부실한 점(법적 충실도·신뢰도)을 한국 표준 기준으로 보강하고, 동작하지 않는 마케팅 동의 체크박스를 YAGNI 원칙에 따라 제거한다.

### 약관·개인정보처리방침 컨텐츠 보강

**이용약관**: 6개 조 → 14개 조로 확장 (~1100자 → ~4200자). 추가된 조항:
- 정의 조항 (서비스/이용자/AI 답변)
- 약관의 효력 및 변경 절차 (시행일 7일 전·중대 변경 30일 전 고지 등)
- 회원 가입 및 계정 관리 (14세 미만 가입 거절, 도용 책임)
- 요금제 및 결제 (토스페이먼츠 명시)
- 구독 해지 및 환불 (전자상거래법·콘텐츠산업진흥법 근거)
- AI 서비스의 한계 및 면책 (전문가 검토 의무, 고의·중과실 면책)
- 지적재산권 (입력물·생성물·서비스 권리 분리)
- 이용자 의무 및 금지 행위 (자동화·역공학·재판매 등)
- 서비스 이용 제한 및 계약 해지
- 서비스 변경 및 중단
- 손해배상 및 책임 한도 (직전 12개월 이용료 한도)
- 준거법 및 분쟁 해결

**개인정보처리방침**: 5개 항목 → 14개 항목으로 확장 (~700자 → ~4400자). 한국 개인정보 보호법 표준 항목 충실히 반영:
- 수집 항목 분류 (회원가입 시·온보딩 선택·자동 수집)
- 이용 목적 6가지 명시
- 보유·이용 기간 (전자상거래법 5년·통신비밀보호법 3개월 등 법령 근거)
- 제3자 제공 조항 (원칙적 미제공 + 예외 명시)
- **처리 위탁** (7개 위탁사 명시: Anthropic·Supabase·Cloudflare·Google·Kakao·Resend·토스페이먼츠)
- **국외 이전** (개인정보보호법 제28조의8 의무 고지 — 이전받는 자·국가·일시·항목·기간 5요소)
- 만 14세 미만 아동 (가입 받지 않음 명시)
- 이용자 권리 (열람·정정·삭제·처리정지·동의철회 + 행사 방법)
- 안전성 확보 조치 (관리적·기술적·물리적)
- 쿠키 사용 (광고 추적 미사용 명시)
- **개인정보 보호책임자** 지정 (법 제31조 의무) — 리팡, privacy@route01.kr
- 권익 침해 구제 방법 (개인정보분쟁조정위·KISA·검찰·경찰 표준 안내)
- 변경 절차

법령 근거 명시(개인정보보호법 제26·28의8·30·31조, 전자상거래법, 콘텐츠산업진흥법, 통신비밀보호법)로 GCP·KISA·고객 신뢰 모두 향상.

### 정적 페이지 — terms.html · privacy.html

**별도 self-contained HTML 페이지** (nachim_v3.css 의존 없음 — 약관 페이지에 SPA CSS 전체 로드는 과함):
- 헤더: Route01 로고 + "← 서비스로 돌아가기" 링크
- 본문: 760px 최대 폭, Inter 폰트, --bg/--bg2/--ink 동일 컬러 토큰 인라인 정의
- 푸터: 홈·이용약관·개인정보처리방침 상호 링크
- 모바일 반응형 (560px 미만), 인쇄 최적화 (`@media print`)
- canonical URL 명시 (`<link rel="canonical">`) — SEO·OAuth 등록용

### URL 라우팅 — `_redirects` 추가
```
/terms      /terms.html      200
/privacy    /privacy.html    200
```
200 redirect는 URL을 유지하면서 내부 리라이트 — `route01.kr/terms`로 접근 시 주소 그대로 terms.html 컨텐츠 서빙. GCP·법적 등록은 항상 깔끔한 `https://route01.kr/terms`·`https://route01.kr/privacy` 형태 사용.

### 모달 처리 (의도된 듀얼 시스템)
- **footer 링크** (line 226-227): 정적 페이지 (`/terms`, `/privacy`) — `target="_blank"`로 새 탭. 외부 공유·검색·OAuth 등록에 표준화된 URL.
- **온보딩 "보기" 버튼** (`btn-view-terms`, `btn-view-privacy`): 모달 유지. 가입 흐름 끊지 않기 위함. R01_TERMS 객체와 openTermsModal 함수 그대로 둠 — 컨텐츠는 신규 보강된 풀 텍스트 사용.

### 마케팅 동의 체크박스 제거 — YAGNI

제거 이유:
- 동작하는 채널 없음 (newsletter·SMS·push 모두 미구축)
- DB 컬럼 없음 (`profiles`에 `marketing_consent` 없음)
- 체크/미체크가 어떤 동작도 변경시키지 않음
- "마케팅 정보 수신"이 무엇인지 사용자에게 설명 없음

제거 범위:
- HTML: `<input id="agree-marketing">` row 삭제
- JS `toggleAllTerms`: `'agree-marketing'` 배열에서 제거
- JS `syncAllCheck`: `agree-marketing` 체크 조건 제거
- JS row click handler (line 6221): `'agree-marketing'` OR 조건 제거

마케팅/Newsletter 채널 트랙은 메모리에 별도 기록 — 채널 생성 시점에 체크박스 + DB 컬럼 + 동의 시각 기록 + 약관 업데이트 일괄 추가.

### 코드 리뷰
- ✅ JS 문법 (`node -c`)
- ✅ HTML 태그 균형 (index 538/538, terms 100/100, privacy 126/126)
- ✅ 마케팅 참조 0건 (자문 도메인 카테고리 `marketing` key는 정상 — 별개)
- ✅ 캐시 버스터 일치 (CSS+JS 둘 다 v39)
- ✅ openTermsModal 함수·온보딩 핸들러 정상 유지

### 외부 작업 (리팡님 직접) — 배포 확인 후

1. **Cloudflare Pages 배포 확인**: route01.kr/terms, route01.kr/privacy 접근 가능한지 확인
2. **Google Cloud Console 브랜딩 등록** (Auth Platform → 브랜딩):
   - 앱 로고: route01.kr/logo.png 업로드
   - 앱 홈페이지 URL: `https://route01.kr`
   - 개인정보처리방침 URL: `https://route01.kr/privacy`
   - 서비스 약관 URL: `https://route01.kr/terms`
   - 승인된 도메인 확인 (§48에서 등록됨)
3. (선택) Search Console 도메인 검증 — 더 빠른 OAuth 화면 반영을 위해

### 다음 세션 우선순위 (§50 추천)

**1순위 — 토스페이먼츠 결제 연동 + TEMP 코드 정리**
- §47/§48에서 결정된 "방향 A"의 결제 부분 (마지막 큰 트랙)
- TEMP 마커 4곳 정리 (line 247, 344, 6450, 6484)
- 한 세션 전체 분량
- 약관 §6·§7·개인정보처리방침 위탁 항목에 토스페이먼츠 이미 명시 — 약관 추가 작업 불필요

**그 외 트랙**:
- 디자인 리뉴얼 (Claude Designer 출시 시) — UI/UX + 답변 화면 렌더링(표·폰트·영문·**볼드 등) + 마이페이지/비밀번호 모달
- Naver Edge Function bridge (베타 후)
- Apple 로그인 활성화 (글로벌 런칭)
- Newsletter 채널 + 마케팅 동의 재도입
- 카카오 비즈 앱 → 정식 사업자 인증 전환

### §49 커밋 정리 — 1 커밋, 캐시 버스터 v38 → v39
| 커밋 | 내용 | 캐시 버스터 |
|---|---|---|
...
> **참고**: 아래 "마감 — 세션 통계 / 검증 결과" 섹션은 동일 마감의 두 번째 작성분.
> 위 섹션이 학습 사항·외부 작업·교훈 중심이고, 아래는 통계·검증·이연 트랙 중심.
> 인터넷 연결 일시 끊김으로 인해 두 차례 마감 처리된 결과물이지만, 정보가 상호 보완적이라 둘 다 유지함.

---

## §49 마감 — 세션 통계 / 검증 결과 (2026-05-04 EOD)
